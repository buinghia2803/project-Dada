...
import {CONTRACT_STATUS, OFFER_STATUS} from "~/utils/constants";
...
export default {
  ...
  methods: {
    ...
    replaceQuery( data, merged = true ) {
       ...
       * @returns {*}
       */
      getOfferStatus(contractOffer = {}) {
          let statusId = null;
          switch (contractOffer.status) {
              case 0:
                  statusId = 2;
                  break;
              case 1:
          ...
          const statusLabel = OFFER_STATUS.find(element => element.id == statusId);
